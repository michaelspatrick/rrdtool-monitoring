...
IMAGEPREFIX="apache_requests"
DBFILE=$DBDIR/$IMAGEPREFIX.rrd
VERTICALLABEL="Requests/sec"
TITLE="Apache Requests/sec"
DBVAR="requests"
DBVARTITLE="$ACCESSLOG_DOMAIN"
...
rrdtool graph $IMGDIR/${IMAGEPREFIX}_one_hour.png -s $ONE_HOUR_AGO -h $GRAPHHEIGHT -w $GRAPHWIDTH -a PNG -t "$TITLE (last 1 hour)" \
	...
	--alt-autoscale-max \
	--lower-limit=0 \
	DEF:a=$DBFILE:$DBVAR:AVERAGE \
	LINE2:a#6666FF:"$DBVARTITLE" \
	GPRINT:a:LAST:"Current\:%5.2lf"  \
	GPRINT:a:AVERAGE:"Average\:%5.2lf"  \
	GPRINT:a:MAX:"Maximum\:%5.2lf"
