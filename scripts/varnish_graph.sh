...
rrdtool graph $IMGDIR/${IMAGEPREFIX}_one_hour.png -s $ONE_HOUR_AGO -h $GRAPHHEIGHT -w $GRAPHWIDTH -a PNG -t "$TITLE (last 1 hour)" \
	...
	--alt-autoscale-max \
	--lower-limit=0 \
	DEF:a=$DBDIR/varnish.rrd:hitrate:AVERAGE \
	LINE2:a#6666FF:"HitRate" \
	GPRINT:a:LAST:"Current\:%4.2lf"  \
	GPRINT:a:AVERAGE:"Average\:%4.2lf"  \
	GPRINT:a:MAX:"Maximum\:%4.2lf"
