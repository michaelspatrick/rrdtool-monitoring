...
rrdtool graph "$IMGDIR/${IMAGEPREFIX}_one_hour.png" -s $ONE_HOUR_AGO -h $GRAPHHEIGHT -w $GRAPHWIDTH -a PNG -t "$TITLE (last 1 hour)" \
	...
	--rigid \
	--units-exponent 0 \
	DEF:a="$DBDIR/load.rrd":user:AVERAGE \
	DEF:b="$DBDIR/load.rrd":nice:AVERAGE \
	DEF:c="$DBDIR/load.rrd":system:AVERAGE \
	DEF:d="$DBDIR/load.rrd":iowait:AVERAGE \
	DEF:e="$DBDIR/load.rrd":idle:AVERAGE \
	AREA:a#99FF66:"USER"  \
	STACK:b#6666FF:"NICE"  \
	STACK:c#FF3333:"SYSTEM" \
	STACK:d#EACC00:"IOWAIT" \
	STACK:e#666666:"IDLE" 
